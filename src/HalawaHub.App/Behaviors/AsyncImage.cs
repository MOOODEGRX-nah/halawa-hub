...
public static class AsyncImage
{
    ...
    // عرض فك الترميز بالبكسل — يحدد لكل مكان استخدام حجم يناسب حجمه الفعلي
    // على الشاشة بدل حجم موحّد كبير للكل (يوفر رام كبير عند وجود بطاقات صغيرة
    // زي صور الشريط الجانبي 28×28). القيمة الافتراضية 300 لو ما تحدد.
    public static readonly AttachedProperty<int> DecodeWidthProperty =
        AvaloniaProperty.RegisterAttached<Image, int>("DecodeWidth", typeof(AsyncImage), 300);
    ...
    // المفتاح = الرابط + عرض فك الترميز، لأن نفس الغلاف ممكن يُطلب بحجمين
    // مختلفين بمكانين مختلفين بالواجهة (بطاقة صغيرة وتفاصيل كبيرة).
    //
    // v0.0.10.5: صار للكاش حد أقصى (80 صورة) — لما يتعداه، أقدم صورة تنحذف
    // أول بأول (FIFO). بدون هذا الحد، مكتبة فيها 150+ لعبة كانت تخلي الكاش
    // يكبر بلا سقف كل ما المستخدم يسكرول على مكتبته كاملة، وهذا كان أكبر
    // مصدر استهلاك رام متبقي بعد التحميل الكسول.
    private const int MaxCacheEntries = 80;
    private static readonly ConcurrentDictionary<string, Bitmap?> Cache = new();
    private static readonly ConcurrentQueue<string> CacheOrder = new();

    private static void CacheSet(string cacheKey, Bitmap? bitmap)
    {
        ...
        while (CacheOrder.Count > MaxCacheEntries && CacheOrder.TryDequeue(out var oldest))
    ...
    private static async Task StartLoad(Image image, ImageLoadState state, string url, string? fallbackUrl)
    {
        ...
        try
        {
            ...
            // فك ترميز بعرض يناسب حجم هذا العنصر بالضبط (يتحدد من XAML) بدل حجم
            // موحّد كبير للكل — يوفر رام محسوس لما تكون البطاقة صغيرة (زي صور
            // الشريط الجانبي 28×28)
            var bitmap = Bitmap.DecodeToWidth(stream, decodeWidth);

            CacheSet(cacheKey, bitmap);
