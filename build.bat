@echo off
REM ============================================================
REM  يبني RiftClone كبرنامج مستقل (exe واحد) تقدر تشغّله مباشرة
...
echo جاري بناء RiftClone...
echo.

dotnet publish src\RiftClone.App\RiftClone.App.csproj ^
    -c Release ^
    -r win-x64 ^
    --self-contained true ^
...
dotnet build src\RiftClone.Plugins.Sample\RiftClone.Plugins.Sample.csproj -c Release -o build-temp-plugin >nul
copy /Y "build-temp-plugin\RiftClone.Plugins.Sample.dll" "publish\Plugins\" >nul
